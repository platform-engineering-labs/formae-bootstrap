...
#
# Generate the formae agent API basic-auth credential for aws/bootstrap.pkl.
#
...
# your machine. Pass the printed hash to `formae apply --api-password-hash …`; keep
# the printed password for scripts/write-bootstrap-profile.sh (your CLI profile).
#
# Usage: aws/scripts/gen-api-credential.sh [username]   (username defaults to "formae")
...
1. Apply the agent with this credential + your access-mode flags (the hash is stored in
   Secrets Manager and injected at task start, never written into the task def):

   # alb (public HTTPS): also pass --cert-arn <acm-arn> --domain <your-domain>
   # tailnet (private):  also pass --access tailnet --ts-authkey <tskey>
...
2. After it is up, write a connected CLI profile (keep the password above — you need it here):
   ...
   aws/scripts/write-bootstrap-profile.sh --profile bootstrap \\
     --user ${user} --password '${password}' --domain <your-domain>
EOF
